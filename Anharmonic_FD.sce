...
k=100;
b=10;
rmin=1e-10;   //Making a grid of point
rmax=10;
points=100;
...
for i=1:points
   ...
   //potential(i,i)=(-(e^2)*((i*step))^(-1))
    potential(i,i)=((k.*r(i)^(2))/2)+(b*r(i)^(3)/3);
    end
...
E=diag(b);
wavefunction =a(:,1);
wavefunction1=a(:,2);                //Wavefunction is an Eigenvector 
...
plot(r,wavefunction.*wavefunction,'b-','LineWidth',2)
